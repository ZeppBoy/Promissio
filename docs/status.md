...
Working-tree assessment: 2026-09-18, based on `main` commit `57a93bd`. [Phase 2 completion evidence](verification/2026-09-18-phase-2-completion.md) records its checks and sign-off; [Phase 3 aging evidence](verification/2026-09-18-phase-3-aging.md) records the current persistence verification.
...
| Phase 3 loan lifecycle | Aggregate/state machine, idempotent creation/replay, persisted activation and persisted aging with optimistic concurrency | Nine PostgreSQL Testcontainers scenarios pass; terminal transitions, projections and historical queries are not implemented |
...
The Application test project covers creation, activation and aging handlers. Integration contains passing real PostgreSQL scenarios for creation, replay, duplicate handoff, changed-payload conflict, concurrent handoffs, persisted activation, grace, past due, cure, default, the Active no-op and stale-writer rejection. Docker Desktop 4.91.0 is installed on the current host. AI evaluations remain empty.
...
The next bounded implementation candidate is a first read-model projection. Projection lifecycle and event-schema evolution decisions remain required before Phase 3 T6 and need explicit owner approval. [Authorization](adr/0008-single-tenant-authorization.md) still requires explicit security review before exposing operations. See the [roadmap](plan/README.md) for acceptance criteria.
